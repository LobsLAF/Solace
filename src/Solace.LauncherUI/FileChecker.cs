...
internal static class FileChecker
{
    ...
    public static async Task<bool> CheckAsync(Settings settings, bool checkImporter, ILogger logger, CancellationToken cancellationToken)
    {
        ...
        if (settings.UseModdedResourcePack is true)
        {
            var resourcePack = new FileInfo(Path.GetFullPath(Path.Combine(Program.StaticDataDir, "resourcepacks", "modded.zip")));
            if (!resourcePack.Exists)
            {
                logger.Error($"Resourcepack file '{resourcePack.FullName}' does not exist, but 'Use Modded Resource Pack' is enabled.");
                error = true;
            }
        }
        else
